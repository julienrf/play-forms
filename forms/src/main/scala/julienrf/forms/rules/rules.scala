...
case object ToInt extends Rule[String, Int](s => Try(s.toInt), n => Seq(n.toString))
case class Min(n: Int) extends Rule[Int, Int](a => if (a >= n) Success(a) else Failure(Error.MustBeAtLeast(n)), n => Seq(n.toString))
case class Opt[A, B](rule: Rule[A, B]) extends Rule[A, Option[B]](a => Success(rule.run(a).toOption), { case Some(b) => rule.show(b); case None => Seq("") })
case class InMap[A, B](rule: Rule[FieldData, A], f1: A => B, f2: B => A) extends Rule[FieldData, B](d => rule.run(d).map(f1), f2 andThen rule.show)
